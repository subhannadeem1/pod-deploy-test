...
export default function TestChatbot() {
  ...
  // Handle form submission with streaming response and episodes fetch
  const handleSubmit = async (e: FormEvent, overrideQuery?: string) => {
    ...
    try {
      ...
      // Check episodes response
      if (!episodesRes.ok) {
        const text = await episodesRes.text();
        throw new Error(text || "Error from episodes endpoint");
      }
      ...
      if (!answerRes.ok) {
        ...
        throw new Error(text || "Error from answer endpoint");
      }
      if (!answerRes.body) {
      ...
      // Set episodes after answer streaming completes
      ...
      <div className="flex-1 flex flex-col px-2">
        <div className="flex-1 flex flex-col items-center overflow-y-auto justify-start border-b-0 border rounded-t-[15px] bg-[#0A1016] border-[#17212A] pb-10 mt-24">
          ...
          <div className="w-full max-w-4xl space-y-8">
            ...
            {error && (
              <div className="text-red-600 mb-4 mt-4 flex items-center gap-2">
                <p>Error: {error}</p>
                <button
                  onClick={() =>
