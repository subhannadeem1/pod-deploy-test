...
export default function TestChatbot() {
  ...
  useEffect(() => {
    ...
    if (storedHistory) {
      try {
        ...
        localStorage.removeItem("PG_HISTORY"); // Clear invalid data
      }
    }
  }, []);
  ...
  const handleSubmit = async (e: FormEvent, overrideQuery?: string) => {
    ...
    try {
      // Fetch episodes data first
      let episodesData: { episodes: EpisodeResult[] } = { episodes: [] };
      ...
      try {
        ...
        if (!episodesRes.ok) {
          ...
          let errorMessage = "Error fetching episodes";
          ...
          console.error("Episodes API error:", errorMessage);
          // Don't throw here, continue with answer fetch
        } else {
          episodesData = await episodesRes.json();
        }
      } catch (err) {
        console.error("Error fetching episodes:", err);
        // Continue with answer fetch
      }

      // Now fetch the answer with streaming
      try {
        ...
        <div className="flex-1 flex flex-col px-2">
          <div className="flex-1 flex flex-col items-center overflow-y-auto justify-start border-b-0 border rounded-t-[15px] bg-[#0A1016] border-[#17212A] pb-10 mt-24">
            ...
            <div className="w-full max-w-4xl space-y-8">
              {/* Error Message */}
              {error && (
                <div className="text-red-600 mb-4 mt-4 px-4">
                  <p>Error: {error}</p>
                </div>
              )}
